...
def getNickName(name):
    if name  == "Renoir Radeon High Definition Audio Controller Digital Stereo (HDMI)":
        return "HDMI Speakers"
    if name == "Family 17h/19h HD Audio Controller Analog Stereo":
...
def getSinks():
    ...
    sinkDetails['alsa_output.pci-0000_04_00.1.hdmi-stereo'] = 'DP Headphone'
    # sinkDetails['alsa_output.pci-0000_03_00.1.hdmi-stereo'] = 'HDMI Speaker'
    ...
    for sink in sinks:
        ...
        if "Loopback" in name or 'S/PDIF' in name  or 'Navi' in name:
            continue
            print("\n\n\n\nyes\n\n\n\n")
            # sink
        # sinkDetails[sink] = name.rstrip('\n')
        sinkDetails[sink] = getNickName(name.rstrip('\n'))
