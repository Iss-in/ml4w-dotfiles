...
toggle_app() {
    ...
    if [ -z $app_workspace ] ;then
        ...
        echo "app in another workspace ${app_workspace} moving to ${current_workspace}"
        echo \"$current_workspace,address:$address\"
        hyprctl dispatch movewindowpixel   "exact 20% 15%,address:$address" && hyprctl dispatch movetoworkspacesilent "$current_workspace,address:$address" && hyprctl dispatch movewindowpixel   "exact 20% 15%,address:$address"
    else
        echo "$workspace in same workspace $app_workspace, moving to special:$workspace"
        hyprctl dispatch movewindowpixel   "exact 40% 30%,address:$address" &&  hyprctl dispatch movetoworkspacesilent "special:${workspace},address:$address"
    fi
}
