...
weather_desc=$(curl "wttr.in/Kanpur?format='%C'" | sed "s|'||g")
weather_icon=$(curl "wttr.in/Kanpur?format='%c'" | sed "s|'||g")
# weather_temp=$(echo $weather | jq ".main.temp" | cut -d "." -f 1)
weather_temp=$(curl rpi:8000/getTemps | jq .temprature )
pying stackoverflow ansfeels_like=$(curl "wttr.in/Kanpur?format='%f'" | sed "s|'||g")
weather_icon_code=$(echo "$weather" | jq -r ".weather[].icon" | head -1)
# feels_like=$(echo $weather | jq ".main.feels_like" | cut -d "." -f 1)
# humidity=$(echo $weather | jq ".main.humidity" | cut -d "." -f 1)
humidity=$(curl rpi:8000/getTemps | jq .Humidity)
